...
The following options are available:
    ...
    -u [FILENAME]  UMAP mode. Load nodes' coordinates obtained by reducing the dimension of 
                   nodes' feature using UMAP algorithm.
    -m [FRACTION_MISSING_LINKS] The fraction of missing links. The embedding the network used in 
...
void parse_options(int argc , char *argv[], embeddingS1_t &the_graph)
{
  ...
  // Parsing options.
  int opt;
  while ((opt = getopt(argc,argv,"ab:cfko:r:qs:vd:eu:m:")) != -1)
  {
    switch(opt)
    {
      case 'a':
      ...
      case 'e':
        ...
        break;
      case 'u':
        the_graph.UMAP_MODE = true;
        the_graph.ALREADY_INFERRED_PARAMETERS_FILENAME = optarg;
        break;
      case 'm':
        the_graph.FRACTION_MISSING_LINKS = std::stof(optarg);
        break;
      default:
        print_usage();
        print_help();
